class Engine {
  ...
  _getKebapCase(string){
    return string.replace(/\B([a-z0-9]|(?=[A-Z]))([A-Z])/g, '$1-$2').toLowerCase();
  }

  _injectComponent(component) {
    const componentNameKebapCase = this._getKebapCase(component.name);
    const childComponents = this.node.getElementsByTagName(componentNameKebapCase);
